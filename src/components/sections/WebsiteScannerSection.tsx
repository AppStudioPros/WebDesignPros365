...
import { 
  Search, Globe, Shield, Zap, TrendingUp, AlertTriangle, CheckCircle, XCircle,
  ArrowRight, Sparkles, RefreshCw, DollarSign, Clock, Bot, Lock, Rocket, Smartphone, X, Eye
} from 'lucide-react';
import { Button } from '@/components/ui/button';

// Score Card Component
const ScoreCard = ({ icon: Icon, label, score, grade, description, color }: any) => {
...
const IssueItem = ({ severity, title, impact }: any) => {
...
const UpgradeCard = ({ upgradeKey, icon: Icon, title, description, badge, badgeColor, onClick }: any) => (
  <motion.div 
...
const UpgradeDetailModal = ({ upgradeKey, onClose }: any) => {
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {Object.entries(upgrade.stats).map(([key, value]: any) => (
          ...
          <Link href="/contact" onClick={onClose}>
            <Button className="w-full bg-gradient-to-r from-[#8734E1] to-[#BF5DE0] hover:opacity-90">
...
const HUDLoadingAnimation = ({ progress, status, metrics }: any) => {
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
export default function WebsiteScannerSection() {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        const performanceScore = Math.round(data.performance?.score || 0);
        const seoScore = Math.round(data.seo?.score || 0);
        const accessibilityScore = Math.round(data.accessibility?.score || 0);
                ...
                <form onSubmit={handleScan} className="max-w-2xl mx-auto">
                  <div className="relative">
                    ...
                    <div className="absolute right-2 top-1/2 -translate-y-1/2">
                      <Button type="submit" className="bg-gradient-to-r from-[#8734E1] to-[#BF5DE0] hover:opacity-90">
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-500">
                      *Estimates vary based on your traffic, industry, and conversion rates.
                    </p>
                  </motion.div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 mb-6">
                    <Link href="/portfolio">
                      <motion.button
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full py-4 px-6 bg-gradient-to-r from-[#8734E1] to-[#2F73EE] text-white font-semibold rounded-xl flex items-center justify-center gap-2 hover:shadow-lg transition-all"
                      >
